...
async function loadProducts() {
    ...
    if (data) {
        ...
        const list = document.getElementById('product-list'), drop = document.getElementById('standing-item');
        ...
        activeProducts.forEach(p => {
            ...
            list.innerHTML += `<div class="item-row py-4 border-b"><div class="flex justify-between items-center px-2"><div class="text-left"><p class="font-bold text-slate-800 uppercase text-[13px] leading-tight">${p.name}</p><button onclick="toggleNote('${p.name}')" class="text-[9px] font-black text-blue-500 uppercase mt-1">📝 Note</button></div><input type="number" oninput="validateChanges()" data-name="${p.name}" value="0" class="w-16 p-3 bg-slate-50 border-2 rounded-xl text-center font-black text-blue-600 outline-none"></div><input type="text" id="note-${p.name}" oninput="validateChanges()" placeholder="Note for ${p.name}..." class="note-input"></div>`;
...
async function generateConsolidatedReport() {
    ...
    const venueReport = {};
    ["WYN", "MCC", "WSQ", "DSQ", "GJ"].forEach(v => venueReport[v] = { "1st Delivery": [], "2nd Delivery": [] });

    (standings || []).forEach(s => { if(s.days_of_week.includes(targetDay)) venueReport[s.venue_id][s.delivery_slot].push({ name: s.item_name, qty: s.quantity }); });
    (oneOffs || []).forEach(o => { venueReport[o.venue_id][o.delivery_slot] = o.items.map(i => ({ name: i.name, qty: i.quantity, note: i.comment })); });
    ...
    Object.keys(venueReport).forEach(v => {
        ...
        if(vData["1st Delivery"].length > 0 || vData["2nd Delivery"].length > 0) {
            html += `<div class="mb-6 p-4 border-2 rounded-2xl bg-slate-50 text-left"><h3 class="font-black text-blue-800 text-lg border-b mb-3 italic">${v}</h3>`;
...
function renderStandingList() {
    const cont = document.getElementById('standing-items-container'); cont.innerHTML = "";
...
async function addStandingOrder() {
    ...
    if (!item || !qty || days.length === 0) return alert("Fill info.");
...
async function deleteStanding(id) { if(confirm("Remove?")) { await _supabase.from('standing_orders').delete().eq('id', id); loadStandingOrders(); } }
...
function toggleDay(btn) { btn.classList.toggle('day-active'); }
