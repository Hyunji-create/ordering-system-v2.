                    ...
                    <div class="item-row py-4 border-b">
                        <div class="flex justify-between items-center px-2">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <input type="number" id="qty-${p.name}" oninput="validateChanges()" data-name="${p.name}" value="0" inputmode="numeric" pattern="[0-9]*" class="w-12 h-11 border-2 rounded-xl text-center font-black text-blue-600 outline-none border-slate-200">
                                <button type="button" onclick="adjustQty('${p.name}', 1)" class="qty-btn">+</button>
                            </div>
                        </div>
...
function checkFormLock() {
    ...
    if (locked && window.currentUser.role !== 'kitchen') { 
...
window.generateConsolidatedReport = async function() {
    ...
    // Math fix for Advance Prep: looking for orders FOR (Selected Date + 1)
    const prepDateObj = new Date(targetDateObj); prepDateObj.setDate(prepDateObj.getDate() + 1); 
    ...
    try {
        const { data: allOrders } = await _supabase.from('orders').select('*').in('delivery_date', [dateStr, prepDateStr]);
        ...
        // 1. Process Daily Orders (Priority)
        (allOrders || []).forEach(o => {
        ...
        (standings || []).forEach(s => {
            ...
            if (s.days_of_week.includes(prepDay) && LEAD_2_DAY_ITEMS.includes(s.item_name)) {
                const venueHasDailyOnPrep = (allOrders || []).some(o => o.delivery_date === prepDateStr && o.venue_id === s.venue_id && o.delivery_slot === s.delivery_slot);
        ...
        if (Object.keys(leadPrep).length > 0) {
            html += `<div class="mb-6 p-4 bg-orange-50 border-2 border-orange-200 rounded-3xl print:hidden"><h2 class="text-xs font-black text-orange-800 uppercase mb-2 italic">Advance Prep (For Tomorrow ${prepDateStr})</h2>`;
